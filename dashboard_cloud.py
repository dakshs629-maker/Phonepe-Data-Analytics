...
# Helper for currency formatting (Indian Standard)
    ...
    with right:
        ...
        if not api_key:
            ...
            if user_query:
                try:
                    ...
                    # UNIVERSAL FIX: Use the specific model path to avoid 404 errors
                    model = genai.GenerativeModel('models/gemini-1.5-flash')
                    
                    # PRO MBA CONTEXT: Explicitly inject unique services list to prevent omissions
                    ...
                    # PRO MBA PROMPT: Directs the AI to be thorough and analytical
                    prompt = (
                        f"Role: MBA FinTech Analyst. Answer questions based on this data context: {context}. "
                        f"Question: {user_query}. "
                        "Instruction: If asked to list services, you MUST list every category in the Unique_Services_List, "
                        "including Insurance, regardless of its transaction volume or frequency."
                    )
        ...
        with st.expander("📊 Quick Stats", expanded=True):
            ...
            if amount_col:
                ...
                # Dynamic Success Rate Detection
