...
# Priority 1: Check Streamlit Cloud Secrets (Fixed spelling to match your panel)
if "GEMINI_API_KEY" in st.secrets:
    api_key = st.secrets["GEMINI_API_KEY"]
elif "GEMEINI_API_KEY" in st.secrets: # Handling your previous typo just in case
    api_key = st.secrets["GEMEINI_API_KEY"]
else:
...
if df is not None:
    ...
    with right:
        ...
        if not api_key:
            ...
            user_query = st.text_input("Ask a business question:", placeholder="e.g., How many services are offered?")
            if user_query:
                try:
                    genai.configure(api_key=api_key)
                    # FIXED MODEL STRING: Using lowercase 'gemini-1.5-flash'
                    ...
                    prompt = f"Act as an MBA FinTech Analyst. Answer based on this context: {context}. Question: {user_query}"
