...
# Look for secret in Streamlit Cloud, otherwise check sidebar
...
if df is not None:
    ...
    with right:
        ...
        if not api_key:
            ...
            user_query = st.text_input("Ask a business question:", placeholder="e.g., Which service is most frequent?")
            if user_query:
                try:
                    # INITIALIZING THE AI
                    genai.configure(api_key=api_key)
                    
                    # UNIVERSAL MODEL IDENTIFIER
                    model = genai.GenerativeModel('gemini-1.5-flash')
                    
                    # PREPARING BUSINESS CONTEXT
                    context = {
                        "Total Revenue": indian_format(total_rev),
                        "Volume": total_txns,
                        "Unique Services": df['Service'].unique().tolist() if 'Service' in df.columns else "N/A"
                    }
                    
                    prompt = f"Role: MBA FinTech Analyst. Answer questions using this data: {context}. User Question: {user_query}."
                    
                    # GENERATING RESPONSE
                    response = model.generate_content(prompt)
                    st.markdown(f'<div class="ask-box"><b>AI Analyst:</b><br>{response.text}</div>', unsafe_allow_html=True)
                except Exception as e:
                    # CATCHING ERRORS PROFESSIONALLY
        ...
        with st.expander("📊 Quick Stats", expanded=True):
            ...
            if amount_col:
                ...
                if 'Service' in df.columns:
                    st.markdown(f"• **Primary Channel:** {df['Service'].value_counts().index[0]}")

else:
    st.info("👋 Dataset not found. Please upload a CSV to begin.")
